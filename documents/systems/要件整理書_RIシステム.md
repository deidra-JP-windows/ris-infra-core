...
- WEBサイトは外部向けと内部向け両方欲しい（S3を使用）
  - 外部向け WEB サイト (Infoサイト) 
    - 誰でもアクセスできる
      - 動作確認はGithub Pages を使用
        - dev とは別、結合テストをコスト気にせずできるようにする為
...
他システムとの共用にはなるもの、現状以下はマストとします。
- ブランチルールの設定
- シークレット情報のべた書き禁止
- リポジトリシークレットを使用
- CI/CD の無理のない実装（Github Actions 使用）
  - CI
    - マージをブロックするかは状況次第だが基本実装
  - CD
    - 権限の払いだしを行いリポジトリシークレットに都度登録
      - 権限のみ手動対応とし、残りは自動化します。
- バージョン管理をシンプルにし、バージョン切り替えと仮想環境管理のトイルを減らすため、各リポジトリでランタイムを言語別に一つとする。


## Discord アカウント運用方針
デフォルトロールの削除と、ロールによる操作制限を実施。参加ユーザへの特定操作の必須化などを整備する。
